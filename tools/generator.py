...
OUTPUT_PATH = BASE_PATH / MODULE_NAME
...
class PydanticBuilder(Builder):
    ...
    def write_ref(self, prop_object: dict[str, Any]):
        ...
        if is_submodule:
            ...
            src_module = f"from {MODULE_NAME} import {sub_module}"
        ...
        *,
        ...
        if has_types:
            self.import_modules.add(
                f"from {MODULE_NAME}.{domain.lower()}.types import *"
            )
    ...
    for domain in protocol_domains:
        ...
        with (domain_path / "__init__.py").open("w") as init_fp:
            ...
            if "types" in domain:
                has_types = True
                types = domain["types"]
                init_fp.write(f"from {MODULE_NAME}.{domain_name}.types import *\n")
            ...
            if "commands" in domain:
                ...
                init_fp.write(f"from {MODULE_NAME}.{domain_name}.commands import *\n")
            ...
            if "events" in domain:
                ...
                init_fp.write(f"from {MODULE_NAME}.{domain_name}.events import *\n")
